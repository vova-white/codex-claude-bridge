...
export async function runMcpServer(paths: StatePaths, cliPath: string): Promise<void> {
  ...
  server.registerTool(
    "task_status",
    {
      ...
      description:
        "Current status of a task and its executions: queued, running (reason waiting_for_capacity while Claude waits for subscription capacity, or waiting_for_children while nested agents Claude started still run after its turn), completed, failed (reason authentication, subscription_limit, invalid_request, or provider_error, with an error message and action), or interrupted (the service stopped while it ran). A publishing task shows its publication as last checked. Each execution lists the nested agents Claude Code reported (nested), with their status, summary, and how their end is known (termination).",
