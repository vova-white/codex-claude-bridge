...
In read-only tasks, Claude and its nested agents run with the edit tools disabled, but shell commands remain available for inspection. This is a tool policy, not a sandbox: the bridge compares the checkout (HEAD, staged, and working files) before and after the task and lists any change in `result.workspace.modifiedFiles` and `result.failures`, or, when the execution failed or was cancelled and `result` is `null`, in `detail.modifiedFiles`. Writing tasks report their changes in `workspace.changedFiles` and `workspace.commits` instead: in `result.workspace` when completed, in `detail.workspace` when failed or cancelled. `detail.workspace` shows only the first few; for the rest, `task_result` of that execution returns `result: null` with `workspace` paged like a result, following `truncated.next`.
