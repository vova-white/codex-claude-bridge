...
- `failed`: `reason` is `authentication` (not a verified subscription login, detected before the brief is sent, or an authentication error from Claude Code), `subscription_limit`, `invalid_request` (for example an unavailable model), `session_unavailable` (a follow-up whose session cannot be resumed), or `provider_error`. `error.message` reads `Execution failed with <reason>: <cause>.` The cause is composed by the bridge: a check it made before sending anything (no verified subscription login, a model Claude Code does not offer, no session to continue, Claude Code not found), the error code or result subtype Claude Code reported, the exit code or signal of its process, or a timeout. For `subscription_limit` the message adds when capacity resets, when known; if the task changed the checkout, it gives the number of files, listed in `error.modifiedFiles`. It never contains Claude Code's own output. Relay `error.message` and `error.action`. The action depends on the failure: sign in or fix the credential setup, choose another model, wait for the reset, or start a new task; after Claude Code began the task, it tells the user to run `claude` in the project and `/resume` the session (the task's `sessionId`) to see the full output. Do not resubmit in a loop.
