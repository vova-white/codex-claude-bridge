...
- For a writing task, `detail.workspace` soon gives the worktree's commit and changed-file counts, and `task_result` of that execution lists them in pages; for a publishing task `detail.publication` shows what the remote holds (`task_status` also shows the task's `publication`). Wait for them with `task_status` before deciding; the bridge only reads the remote.
