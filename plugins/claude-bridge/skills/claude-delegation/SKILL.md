...
- For a writing task, `detail.workspace` soon lists the worktree's commits and changed files, and for a publishing task `detail.publication` shows what the remote holds (`task_status` also shows the task's `publication`). Wait for them with `task_status` before deciding; the bridge only reads the remote.
